...
type TaskNode struct {
	...
	AssignedInstance string         `json:"assigned_instance,omitempty" yaml:"assigned_instance,omitempty"`
	ExecutionNode    string         `json:"execution_node,omitempty" yaml:"execution_node,omitempty"`
	LeaseEpoch       int64          `json:"lease_epoch,omitempty" yaml:"lease_epoch,omitempty"`
	ChatContext      string         `json:"-" yaml:"-"` // transient: chat conversation for re-dispatch after amendment
}

func (t *TaskNode) TargetProfile() string {
	if t == nil {
		return ""
	}
	if t.Profile != "" {
...
func (t *TaskNode) ExecutionTarget() string {
	if t == nil {
		return ""
	}
	if t.AssignedInstance != "" {
		return t.AssignedInstance
	}
	return t.TargetProfile()
}
